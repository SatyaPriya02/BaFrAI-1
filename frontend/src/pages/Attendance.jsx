...
export default function Attendance() {
  ...
  // 🔥 Extract loadMine so we can reuse it
  async function loadMine() {
    ...
    // If latest record is checked-in but not checked-out, resume timer
    if (res.data?.length) {
  ...
  const socket = useMemo(
    ...
    const load = async () => {
      ...
      console.log("Attendance data:", res.data); // 🔥 check roles coming in
  ...
  async function submitPhoto(base64) {
    try {
      ...
      if (action === "checkin") {
        ...
        setCheckInTime(null); // stop timer on checkout
        setWorkTimer(null);
      }
              ...
              <thead>
                <tr style={{ background: "#f0f4f8" }}>
                  {["#", "Emp ID", "Name", "Role", "Check-In", "Check-Out", "Worked"].map((h, idx) => (
                    <th key={idx} style={{ border: "1px solid #ddd", padding: "10px", textAlign: "center" }}>
                      {h}
                    </th>
                  ))}
                </tr>
